...
import org.wso2.carbon.mediator.dblookup.multiple.MultipleDBLookupMediatorFactory;
 ...
 * Time: 10:53 AM
 */
public class MultipleDBLookupMediatorSerializer extends AbstractMediatorSerializer {
    @Override
    protected OMElement serializeSpecificMediator(Mediator mediator) {
        ...
        OMElement dbMultipleLookupElement = fac.createOMElement("dbMultipleLookup", synNS);
        saveTracingState(dbMultipleLookupElement,mediator);
        serializeDBInformation(multipleDBLookupMediator, dbMultipleLookupElement);
        return dbMultipleLookupElement;

    }

    protected void serializeDBInformation(MultipleDBLookupMediator mediator, OMElement dbParent) {
    ...
    private OMNode createPoolElement(MultipleDBLookupMediator mediator) {
        ...
        for (Object o : mediator.getDataSourceProps().keySet()) {
            ...
            if (o instanceof QName) {
                ...
                OMElement elt = fac.createOMElement(
                        MultipleDBLookupMediatorFactory.PROP_Q.getLocalPart(), synNS);
    ...
    private OMNode createStatementElement(Statement statement) {
        ...
        OMElement stmntElt = fac.createOMElement(
                MultipleDBLookupMediatorFactory.STMNT_Q.getLocalPart(), synNS);

        OMElement sqlElt = fac.createOMElement(
                MultipleDBLookupMediatorFactory.SQL_Q.getLocalPart(), synNS);
        ...
        for (String name : statement.getResultsMap().keySet()) {
            ...
            OMElement resultElt = fac.createOMElement(
                    MultipleDBLookupMediatorFactory.RESULT_Q.getLocalPart(), synNS);
    ...
    private OMElement createStatementParamElement(Statement.Parameter param) {
        OMElement paramElt = fac.createOMElement(
                MultipleDBLookupMediatorFactory.PARAM_Q.getLocalPart(), synNS);
